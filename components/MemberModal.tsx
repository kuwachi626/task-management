...
const MemberModal: React.FC<MemberModalProps> = ({ isOpen, onClose, onSave, initialMember }) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4 overflow-y-auto">
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300 my-auto">
        <div className="px-10 py-8 bg-[#0F172A] text-white flex justify-between items-center">
          <h3 className="text-2xl font-black tracking-tight">{initialMember ? 'プロフ編集' : '新規メンバー'}</h3>
          ...
          <div className="flex flex-col items-center gap-4 mb-8">
            <div className="w-24 h-24 rounded-[2rem] bg-slate-100 flex items-center justify-center text-4xl shadow-inner border border-slate-200">
              {formData.avatar.startsWith('http') ? (
                <img src={formData.avatar} alt="" className="w-full h-full object-cover rounded-[2rem]" />
              ) : (
                formData.avatar || '👤'
              )}
            </div>
            <div className="w-full">
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest text-center">プロフィール絵文字</label>
              <input
                type="text"
                ...
                maxLength={100}
              />
              <p className="text-[10px] text-slate-400 text-center mt-2 font-bold uppercase tracking-tight">お好きな絵文字を入力してください</p>
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">氏名</label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">所属</label>
                <input
                  type="text"
                  required
                  className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-amber-500 outline-none text-slate-800 font-bold"
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">役割</label>
                <input
                  type="text"
                  className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-amber-500 outline-none text-slate-800 font-bold"
                  value={formData.role}
